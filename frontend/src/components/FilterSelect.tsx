...
export default function FilterSelect({ label, value, onChange, children, fluid = true }: FilterSelectProps) {
  return (
    <label className={`flex flex-col gap-1 sm:flex-row sm:items-center sm:gap-2 ${fluid ? 'w-full sm:w-auto' : ''}`}>
      <span className="text-xs font-medium text-stone-500 dark:text-stone-400 whitespace-nowrap">{label}</span>
      <select
        value={value}
        onChange={onChange}
        className="w-full sm:w-auto px-3 py-2 sm:py-1.5 text-base sm:text-sm border border-stone-200 dark:border-stone-700 rounded-lg bg-white dark:bg-stone-900 text-stone-700 dark:text-stone-300 focus:outline-none focus:ring-2 focus:ring-navy-500/30 focus:border-navy-500"
      >
        {children}
      </select>
    </label>
