...
export default function QueryState({ isLoading, error, empty, emptyMessage = 'Nothing matches this selection.', loadingMessage = 'Loading…', onRetry, children }: QueryStateProps) {
  if (isLoading) {
    return (
      <div className="card-surface p-12 text-center" aria-busy="true">
        ...
        <p className="mt-3 text-sm text-stone-500 dark:text-stone-400">{loadingMessage}</p>
      </div>
    );
  }
  if (error) {
    ...
    return (
      <div className="card-surface border-brick-200 bg-brick-50 dark:bg-brick-900/30 p-6" role="alert">
  ...
  if (empty) {
    return <div className="card-surface p-10 text-center text-sm text-stone-500 dark:text-stone-400">{emptyMessage}</div>;
  }
  return <>{children}</>;
}
