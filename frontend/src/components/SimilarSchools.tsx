  ...
  return (
    <div className="card-surface p-4 sm:p-6">
      <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
        <div>
          <h2 className="text-base font-semibold text-stone-900 dark:text-stone-100">Similar schools</h2>
          <p className="text-xs text-stone-500 dark:text-stone-400">Same level, closest in low-income share and size, and nearby{data?.lowIncome != null ? ` (${schoolName}: ${data.lowIncome}% low-income)` : ''}</p>
        </div>
      ...
      <ul className="divide-y divide-stone-100 dark:divide-stone-800">
        {similar.map((s) => (
          <li key={s.id}>
            <Link to={`/schools/${s.id}`} className="flex items-center justify-between gap-3 py-2.5 hover:text-navy-700 dark:hover:text-navy-100 dark:text-navy-200">
              <div className="min-w-0">
                <div className="text-sm font-medium text-stone-900 dark:text-stone-100 truncate">{s.name}</div>
                <div className="text-xs text-stone-500 dark:text-stone-400 truncate">{s.districtName}{s.city ? ` · ${s.city}` : ''}</div>
              </div>
              <div className="text-xs text-stone-500 dark:text-stone-400 text-right whitespace-nowrap">
