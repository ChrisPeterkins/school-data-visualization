...
export default function DistrictsPage() {
  ...
  const { latest: latestYear } = useAvailableYears();
  ...
  const { data: districtPerformance } = useQuery({
    queryKey: ['district-performance', selectedDistrict, latestYear],
    queryFn: () => performanceApi.getPSSAResults({
      districtId: parseInt(selectedDistrict!),
      year: latestYear!,
      level: 'district'
    }),
    enabled: !!selectedDistrict && latestYear != null,
  });
      ...
      <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1">
            {selectedDistrict ? (
              <div key={selectedDistrict} className="card-surface p-6 lg:sticky lg:top-4">
                <h2 className="text-base font-semibold text-stone-900 mb-4">District Performance{latestYear ? ` (${latestYear})` : ''}</h2>
                {districtPerformance ? (
