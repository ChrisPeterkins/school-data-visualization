  ...
  if (schoolLoading || trendsLoading) {
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 text-center">
        ...
        <p className="mt-3 text-sm text-stone-500 dark:text-stone-400">Loading school details...</p>
      </div>
    );
  }
    ...
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="card-surface border-brick-200 bg-brick-50 dark:bg-brick-900/30 p-6">
          ...
          <Link to="/schools" className="mt-2 inline-block text-sm text-navy-600 dark:text-navy-300 hover:text-navy-800 dark:hover:text-navy-100 dark:text-navy-200">
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <nav className="flex flex-wrap items-center gap-x-1.5 gap-y-1 text-sm text-stone-500 dark:text-stone-400 mb-6 min-w-0" aria-label="Breadcrumb">
        <Link to="/schools" className="hover:text-navy-600 dark:text-navy-300 transition-colors">Schools</Link>
        <ChevronRightIcon className="w-3.5 h-3.5 flex-shrink-0" />
        <span>{s.countyName}</span>
        <ChevronRightIcon className="w-3.5 h-3.5 flex-shrink-0" />
        <Link to={`/districts/${s.districtId}`} className="hover:text-navy-600 dark:text-navy-300 transition-colors">{s.districtName}</Link>
        <ChevronRightIcon className="w-3.5 h-3.5 flex-shrink-0" />
        <span className="text-stone-700 dark:text-stone-300 font-medium break-words">{s.name}</span>
      </nav>

      <p className="print-only text-xs text-stone-500 dark:text-stone-400 mb-2">PA School Data · chrispeterkins.com/paschools · printed {new Date().toLocaleDateString()}</p>
          ...
          <div className="min-w-0">
            ...
            <div className="mt-4 flex flex-col gap-2 text-sm">
              <div className="flex items-center gap-2">
                <BuildingOffice2Icon className="w-4 h-4 text-stone-500 dark:text-stone-400 flex-shrink-0" />
                <Link to={`/districts/${s.districtId}`} className="text-stone-600 dark:text-stone-400 hover:text-navy-600 dark:text-navy-300">
                  {s.districtName}
                  <span className="text-stone-500 dark:text-stone-400 ml-1 text-xs">(AUN {s.districtAun})</span>
                </Link>
              </div>
              {location && (
                <div className="flex items-center gap-2">
                  <MapPinIcon className="w-4 h-4 text-stone-500 dark:text-stone-400 flex-shrink-0" />
                  <span className="text-stone-600 dark:text-stone-400">{location}</span>
                </div>
              )}
              <div className="flex flex-wrap items-center gap-2">
                {s.type && <span className="inline-flex px-2 py-0.5 text-xs font-medium rounded-full bg-navy-100 text-navy-700 dark:text-navy-200">{s.type}</span>}
                <span className="text-stone-500 dark:text-stone-400">{s.countyName} County</span>
                {s.enrollment ? <span className="text-stone-500 dark:text-stone-400">· {s.enrollment.toLocaleString()} students</span> : null}
              </div>
            </div>
            ...
            <dl className="mt-5 grid grid-cols-3 gap-3 sm:gap-4 text-sm">
              <div>
                <dt className="text-stone-500 dark:text-stone-400 truncate">{math?.subject === 'Algebra I' ? 'Algebra I' : 'Math'}{activeYear ? `, ${activeYear}` : ''}</dt>
                <dd className="text-lg font-semibold text-navy-700 dark:text-navy-200">{formatPct(math?.percentProficientOrAbove)}</dd>
              </div>
              <div>
                <dt className="text-stone-500 dark:text-stone-400 truncate">{ela?.subject === 'Literature' ? 'Literature' : 'ELA'}{activeYear ? `, ${activeYear}` : ''}</dt>
                <dd className="text-lg font-semibold text-navy-700 dark:text-navy-200">{formatPct(ela?.percentProficientOrAbove)}</dd>
              </div>
              <div>
                <dt className="text-stone-500 dark:text-stone-400 truncate">Growth{activeYear ? `, ${activeYear}` : ''}</dt>
        ...
        <div className="mb-8">
          <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
            <h2 className="text-lg font-bold text-stone-900 dark:text-stone-100 flex items-center gap-2">
              PSSA Results
              <span className="text-sm font-normal text-stone-500 dark:text-stone-400">({activeYear})</span>
            </h2>
      ...
      {pssaResults.some((r) => r.grade && r.grade > 0) && (
        <div className="mb-8 space-y-4">
          <h2 className="text-lg font-bold text-stone-900 dark:text-stone-100">Cohorts</h2>
      ...
      {trends && (
        <div className="space-y-4">
          <h2 className="text-lg font-bold text-stone-900 dark:text-stone-100">Historical Performance Trends</h2>
